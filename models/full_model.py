...
class Optimizer_SGD:
    ...
    def update_params(self, layer):
        ...
        if self.momentum:
            ...
            # Build weight updates with momentum - take previous updates multiplied
            # by retain factor and update with current gradients
            weight_updates = \
    ...
    def regularization_loss(self):
        ...
        for layer in self.trainable_layers:
            ...
            if layer.weight_regularizer_l1 > 0:
                reg_loss += \
                    layer.weight_regularizer_l1 * np.sum(np.abs(layer.weights))

            # L2 regularization - weights
            if layer.weight_regularizer_l2 > 0:
                reg_loss += \
                    layer.weight_regularizer_l2 * np.sum(layer.weights * layer.weights)

            # L1 regularization - biases
            # calculate only when factor greater than 0
            if layer.bias_regularizer_l1 > 0:
                reg_loss += \
                    layer.bias_regularizer_l1 * np.sum(np.abs(layer.biases))

            # L2 regularization - biases
            if layer.bias_regularizer_l2 > 0:
                reg_loss += \
                    layer.bias_regularizer_l2 * np.sum(layer.biases * layer.biases)
...
class Model:
    ...
    def forward(self, X, training):
        ...
        for idx, layer in enumerate(self.layers):
            ...
            # set the output of the last layer to be the output of the model
    ...
    def save(self, path):
        ...
        for layer in model.layers:
            for prop in ['inputs', 'output', 'dweights', 'dbiases', 'dinputs']:
...
#     # Create dataset
#     X, y, X_test, y_test = create_data_mnist('preprocessing/fashion_mnist_images')
#
#     # scale (-1, 1) -> value - (max_value / 2) / (max_value / 2)
#     # scale (0, 1) -> value / max_value
#
#     # Scale features
...
#
#     # Reshape to vectors
#     # This will notify NumPy that we wish to keep the 60,000 samples (first dimension),
#     # but flatten the rest (-1 as the second dimension means that we want to put all of the
#     # sample data in this single dimension into a 1D array).
#     X = X.reshape(X.shape[0], -1)  # or X.reshape(X.shape[0], X.shape[1] * X.shape[2]
...
#
#     # Load the model
#     # when loading a model skip the training process and jump right to evaluate method
...
#     model.save_parameters('fashion_mnist_params.pkl')
#
